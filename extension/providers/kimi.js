/**
 * NemApi - Kimi adapter (kimi.com, kimi.ai, moonshot.cn)
 * Contenteditable / textarea input, modern message container extraction.
 */
(function (global) {
  "use strict";
  ...
  const INPUT = [
    'div[contenteditable="true"][data-testid="msh-chat-input"]',
    'div[contenteditable="true"][placeholder*="Kimi" i]',
    'div[contenteditable="true"][placeholder*="输入" i]',
  ...
  const SEND = [
    ...
    'button[data-testid="msh-chat-send-btn"]',
  ...
  const MESSAGE = [
    '[data-testid="msh-chat-message-item"]',
  ...
  function getMessageEls() {
    ...
    for (const s of MESSAGE) {
      ...
      for (const n of nodes) {
        ...
        if (
          n.querySelector('[class*="user"], [data-role="user"]') &&
          !n.querySelector('[class*="assistant"], [class*="ai"], .markdown')
        )
          continue;
        seen.add(n);
        out.push(n);
  ...
  function readMessageText(msgEl) {
    if (!msgEl) return "";
    const mdContainer =
      msgEl.querySelector(".markdown, [class*='markdown'], .chat-content, [class*='content']") ||
      msgEl;
    let text = "";
    if (B.htmlToMarkdown) {
